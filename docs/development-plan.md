...
Labyrinth Composer 的第一阶段目标不是做完整地图编辑器，而是做一个本地优先、服务独立游戏开发者的空间谜题逻辑编辑器与可达性验证器。
...
4. 设计师根据诊断链和修复建议调整结构。
5. 项目保存为可版本控制的 JSON，并可导出给个人引擎项目或自查记录。
...
- 本地优先：项目文件可读、可 diff、可离线使用；实时协作、云端、账号和权限不属于当前产品计划。
...
- 个人自查/备注数据结构。
- 不做实时多人协作；历史 Yjs 原型已从主仓库删除，架构检查继续防止协作依赖回流。
