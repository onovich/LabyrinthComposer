...
核心架构没有被 UI、Tauri、Yjs、Unity/Godot 或 package artifact 污染。
```
...
- 为了协作预留，把 Yjs 或 presence 状态引入 Workbench 主路径。
...
- Yjs document、client presence、remote cursor。
- Unity / Godot 场景对象引用。
...
### 3.7 Collaboration 仍然是实验层

Phase 4 不应把 collaboration 做成产品主线，除非单独开新阶段。

允许：

- 保留 `packages/collaboration-prototype`。
- 增加实验测试或文档。
- 在 architecture check 中继续禁止主包依赖它。

禁止：

- WorkbenchStore 依赖 Yjs。
- ProjectGraph 直接变成 Y.Doc。
- presence / cursor / remote user 状态进入项目文件。
...
packages/collaboration-prototype -> experimental; no package may depend on it
...
ProjectGraph 仍是唯一项目真相；exports/reports/cache 仍是 artifact；UI 仍只渲染 view model；Desktop/CLI 只做宿主读写；worker 只是执行环境；Yjs 仍是实验层。
