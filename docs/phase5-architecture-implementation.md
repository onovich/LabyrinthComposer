...
Phase 5 的目标是把 Phase 4 的 Beta 主路径推进到 Release Candidate，并为后续真实个人生产使用建立受控扩展边界。

本阶段不是“加更多功能”，而是回答三个问题：

1. 这个工具能不能被稳定打包、验证、交付？
2. `.lcproj`、assets、export targets、CI、用户文档是否有可长期维护的契约？
3. 后续扩展和引擎集成是否会污染 `ProjectGraph`、core、schema、workbench 的边界？
...
- 清理已取消的 collaboration prototype 和产品化计划，不在 Phase 5 偷偷并入主路径。
...
- 为了已取消的实时协作方向演示，把 provider/presence 默认接入 WorkbenchStore。
- 为了插件扩展，让 plugin 直接 import core internals 或 UI component。

Phase 5 的一句话原则：

```text
Release 是流程，extensions 是 adapter，assets 是引用，app-local state 是宿主状态；它们都不能成为新的项目真相。
```
...
Phase 5 不产品化 collaboration，也不保留协作原型作为候选方向。

要求：

- 删除旧 collaboration prototype。
- 删除 Phase 6 / Phase 7 协作产品化文档。
- 保留 architecture guard，防止 `yjs`、provider、presence、cursor、room、session state 回流主路径。

禁止：

- WorkbenchStore 默认依赖协作 provider。
...
- 删除旧 collaboration prototype。
- 删除协作产品化文档和 docs check 必需项。
- 更新 architecture check，继续禁止协作依赖回流。
- 确认 lockfile 不再保留 `yjs`。

架构要求：

- 不把 provider 引入 WorkbenchStore。
- 不把 presence、cursor、room、session state 写入 project file。
- 不默认启用任何协作入口。

验收：

- 主 desktop path 不加载 collaboration provider。
- architecture check 继续禁止主包依赖 collaboration package/provider。
...
- collaboration package/provider 不进入主包依赖。
...
collaboration packages/providers -> not planned; no main package may depend on them
...
把 CI、RC artifact、package manifest/assets、export target registry、app-local preferences 做成受控边界。
ProjectGraph 仍然只保存领域真相；release 只验证不发布；assets 是引用不是 blob；export targets 是派生物；preferences 是宿主状态；实时协作方向已取消，只保留架构守卫防止回流。
新增能力前先更新边界和测试，再接 UI。
```
