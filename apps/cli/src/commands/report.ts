...
function parseReportArgs(args: string[]): ParseArgsResult {
...
export async function runReport(rawArgs: string[], io: CliIo): Promise<number> {
  const parsedArgs = parseReportArgs(rawArgs);
  ...
  if (parsedArgs.args.outFile !== undefined) {
    try {
      await writeOutputText(
        ...
        parsedArgs.args.format === 'json' ? 'jsonReport' : 'markdownReport',
        text
      );
    } catch (error) {
